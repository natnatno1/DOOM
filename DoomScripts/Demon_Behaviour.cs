...
public class Demon_Behaviour : MonoBehaviour
{
    public int Health;
    ...
    void Update()
    {
        ...
        // Define what happens when the "DemonAttackForward" animation is playing for this gameobject

        if (Anim.GetCurrentAnimatorStateInfo(0).IsName("DemonAttackForward"))
        {
            ...
            if (AttackCooldown <= 0)
            {
                ...
                GM_Script.Health -= ((Random.Range(1, 10)) * 4);

                // Reset the "AttackCooldown" variable to 0.625 (the length of the attack animation)

                AttackCooldown = 0.625f;
            }
        }
    }
    ...
    {
        // Set the "Hit" variable in the animator to 1.025 (the length of the hit animation + 1)
